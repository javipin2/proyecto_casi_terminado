...
import 'registro/superuser_registro.dart';
...
class SuperAdminDashboardScreen extends StatefulWidget {
  ...
  @override
  SuperAdminDashboardScreenState createState() => SuperAdminDashboardScreenState();
}
...
class SuperAdminDashboardScreenState extends State<SuperAdminDashboardScreen>
  ...
  // Configuraciones de las secciones - ACTUALIZADAS
  final List<Map<String, dynamic>> _sections = [
    ...
    },
    {
      ...
      'title': 'Registro',
      'screen': const SuperuserRegistroReservasScreen(),
      'hasNotification': false,
    },
    {
  ...
  Future<void> _registerAdminTokenForAuditNotifications() async {
    ...
    // Si el usuario tiene lugar pero no ciudad (ej. superadmin con Ciudad: null), obtener ciudad del lugar.
  ...
  Future<void> _navigateToSection(String title, Widget screen) async {
    // 🔐 Bloquear Auditoría avanzada por plan (solo PRO)
    if (title == 'Auditoría') {
      final allowed = await PlanFeatureService.ensureFeatureAvailable(
        context,
        PlanFeature.auditoriaAvanzada,
      );
      if (!allowed) return;
    }
  ...
  Future<void> _showEditNameDialog(BuildContext context) async {
    ...
    final user = FirebaseAuth.instance.currentUser;
    final currentName =
        auth.userName ?? _getAccountName(user, fallback: 'Superusuario');
  ...
  // NUEVA función para menú colapsado - centrado perfectamente
  Widget _buildCollapsedMenuItem(IconData icon, bool isSelected, int? notificationCount) {
    return SizedBox(
      width: 56, // Ancho fijo para centrado perfecto
      child: Stack(
        children: [
          // Icono centrado
          Center(
  ...
  // NUEVA función para menú expandido
